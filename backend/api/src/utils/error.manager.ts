...
const statusMessages: Record<keyof typeof HttpStatus, string> = {
    ...
    NOT_ACCEPTABLE: "Server cannot produce a response matching the list of acceptable values",
    ...
    PRECONDITION_FAILED: "Server does not meet one of the preconditions that the requester put on the request",
    PAYLOAD_TOO_LARGE: "Request payload is too large",
    URI_TOO_LONG: "The URI provided was too long to process",
    UNSUPPORTED_MEDIA_TYPE: "Request entity has a media type which the server or resource does not support",
    EXPECTATION_FAILED: "Server could not meet the expectation given in the Expect request-header field",
    ...
    BAD_GATEWAY: "Server received an invalid response while acting as a gateway or proxy",
    SERVICE_UNAVAILABLE: "Server is currently unavailable",
    GATEWAY_TIMEOUT: "Server did not receive a timely response from the upstream server",
    HTTP_VERSION_NOT_SUPPORTED: "Server does not support the HTTP protocol version used in the request",
    ...
    PRECONDITION_REQUIRED: "Precondition required, please fulfill preconditions first",
    PROCESSING: "Processing the request, hold on",
    REQUESTED_RANGE_NOT_SATISFIABLE: "Requested range cannot be satisfied, please request a valid range",
    TOO_MANY_REQUESTS: "Too many requests, slow down",
    UNPROCESSABLE_ENTITY: "Unprocessable entity, request cannot be processed due to semantic errors",
};
